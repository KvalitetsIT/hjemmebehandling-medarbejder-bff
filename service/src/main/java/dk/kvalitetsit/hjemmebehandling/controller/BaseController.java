...
public abstract class BaseController {
    protected RuntimeException toStatusCodeException(Exception e) {
        return switch (e) {
            case ServiceException s -> toStatusCodeException(s);
            case AccessValidationException ignored -> toStatusCodeException();
            default -> throw new InternalServerErrorException(ErrorDetails.INTERNAL_SERVER_ERROR);
        };
    }
    ...
    private RuntimeException fromErrorDetails(ErrorDetails e) {
        return switch (e) {
            case CAREPLAN_EXISTS, CAREPLAN_ALREADY_FULFILLED,
                 QUESTIONNAIRES_MISSING_FOR_CAREPLAN,
                 UNSUPPORTED_SEARCH_PARAMETER_COMBINATION,
                 PARAMETERS_INCOMPLETE,
                 CAREPLAN_HAS_UNHANDLED_QUESTIONNAIRERESPONSES,
                 CAREPLAN_IS_MISSING_SCHEDULED_QUESTIONNAIRERESPONSES,
                 QUESTIONNAIRE_IS_IN_ACTIVE_USE_BY_CAREPLAN,
                 PLANDEFINITION_IS_IN_ACTIVE_USE_BY_CAREPLAN,
            ...
            case CAREPLAN_DOES_NOT_EXIST,
                 ...
                 QUESTIONNAIRE_RESPONSE_DOES_NOT_EXIST,
                 QUESTIONNAIRE_DOES_NOT_EXIST -> throw new ResourceNotFoundException(e);
            case ACCESS_VIOLATION,
                 MISSING_SOR_CODE -> throw new ForbiddenException(e);
